...
def parse_args(input_args=None):
    ...
    parser.add_argument("--additional_text_encoder_path", type=str, default=None)
...
def main(args):
    ...
    if args.additional_unet_path is not None:
        unet_ckpt = torch.load(args.additional_unet_path)
        if "unet" in unet_ckpt:
            unet.load_state_dict(unet_ckpt["unet"])
        else:
            unet.load_state_dict(torch.load(args.additional_unet_path))
        del unet_ckpt
    if args.additional_text_encoder_path is not None:
        text_encoder_ckpt = torch.load(args.additional_text_encoder_path)
        if "text_encoder" in text_encoder_ckpt:
            text_encoder.load_state_dict(text_encoder_ckpt["text_encoder"])
        else:
            text_encoder.load_state_dict(torch.load(args.additional_text_encoder_path))
        del text_encoder_ckpt
